...
function dizme_tm_trigger_menu() {
  ...
  var mobileMenu = jQuery(".sec_mobile_menu .dropdown");
  var mobileMenuList = jQuery(
    ".sec_mobile_menu .dropdown .dropdown_inner ul li a"
  );
...
function dizme_tm_modalbox_portfolio() {
  ...
  var modalBox = jQuery(".dizme_tm_modalbox");
  var button = jQuery(".sec_portfolio .portfolio_popup");
  var closePopup = modalBox.find(".close");
...
function sec_portfolio() {
  ...
  if (jQuery().isotope) {
    // Needed variables
    var filter = jQuery(".sec_portfolio .portfolio_filter ul");
...
function dizme_tm_projects() {
  ...
  jQuery(".sec_portfolio_animation_wrap").each(function () {
    jQuery(this)
      .on("mouseenter", function () {
        if (jQuery(this).data("title")) {
          jQuery(".sec_portfolio_titles").html(
            jQuery(this).data("title") +
              '<span class="work__cat">' +
              jQuery(this).data("category") +
              "</span>"
          );
          jQuery(".sec_portfolio_titles").addClass("visible");
        }

        jQuery(document).on("mousemove", function (e) {
          jQuery(".sec_portfolio_titles").css({
            left: e.clientX - 10,
            top: e.clientY + 25,
          });
        });
      ...
      .on("mouseleave", function () {
        jQuery(".sec_portfolio_titles").removeClass("visible");
      });
  });
}
...
function dizme_tm_owl_carousel() {
  ...
  var carousel = jQuery(".sec_testimonials .owl-carousel");
...
function dizme_tm_down() {
  ...
  var topbar = jQuery(".sayan_header").outerHeight();
